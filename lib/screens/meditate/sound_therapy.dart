...
class _WavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < bars.length; i++) {
      ...
      ).createShader(Rect.fromLTWH(x, y, barWidth.clamp(1, 100), barH));
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(x, y, barWidth.clamp(1, 100), barH),
...
class _SoundTherapyScreenState extends State<SoundTherapyScreen>
  ...
  double get _progress {
    ...
    final totalSecs = _realDuration.inSeconds > 0
        ? _realDuration.inSeconds.clamp(1, _uiMaxSeconds)
        : _uiMaxSeconds;
    final posSecs = _position.inSeconds % _uiMaxSeconds;
    return (posSecs / totalSecs).clamp(0.0, 1.0);
  }
  ...
  String get _positionStr {
    ...
    if (_realDuration.inSeconds > 0) {
      final loopPos = Duration(
          seconds: _position.inSeconds % _realDuration.inSeconds.clamp(1, 9999));
  ...
  void _skipTo(int delta) {
    ...
    if (idx == -1) return;
    final next = (idx + delta).clamp(0, kSounds.length - 1);
